jQuery(function ($) {
  ...
  if (
    ...
    typeof $.position.scrollbarWidth != "undefined"
  ) {
    document.documentElement.style.setProperty(
      "--scrollbarWidth",
      $.position.scrollbarWidth() + "px"
    );
  }
  $(window).on("ts_slider_middle_navigation_position", function (e, swiper) {
    if (
      swiper.parents(".ts-slider.rows-1").length ||
      swiper.parents(".related").length ||
      swiper.parents(".upsells").length ||
      swiper.parents(".cross-sells").length
    ) {
      var thumbnail = swiper
        .find(".swiper-slide-active")
        .first()
        .find(
          ".product-wrapper .thumbnail-wrapper, .product-wrapper > a, .article-content .thumbnail-content > a.thumbnail"
        );
  ...
  var carousel_data = [
    [
      ".single-product .related .products, .single-product .upsells .products, .woocommerce .cross-sells .products",
      null,
      null,
      typeof ecomall_params != "undefined"
        ? ecomall_params.slider_options
        : null,
    ],
    [
  ...
  $.each(carousel_data, function (i, data) {
    $(data[0]).each(function (index) {
      ...
      var unique_class =
        "swiper-theme-" + Math.floor(Math.random() * 10000) + "-" + index;
      ...
      if (typeof data[3] != "undefined" && data[3] != null) {
        ...
        if (
          typeof extra_settings.show_nav != "undefined" &&
          extra_settings.show_nav
        ) {
          swiper.append(
        ...
        if (
          typeof extra_settings.show_dots != "undefined" &&
          extra_settings.show_dots
        ) {
        ...
        if (
          typeof extra_settings.auto_play != "undefined" &&
          extra_settings.auto_play
        ) {
  ...
  if (
    typeof ecomall_params != "undefined" &&
    ecomall_params.sticky_header == 1
  ) {
    ts_sticky_menu();
  }
  if (
    typeof ecomall_params != "undefined" &&
    ecomall_params.menu_overlay == 1
  ) {
    $(".ts-header .ts-menu")
  ...
  $(document).on("click", "a.quickshop", function (e) {
    ...
    $.ajax({
      ...
      data: {
        action: "ecomall_load_quickshop_content",
        product_id: product_id,
        security: ecomall_params.quickshop_nonce,
      },
  ...
  if (
    ...
    quickshop_css +=
      "body.opening-quickshop{padding-right: " +
      $.position.scrollbarWidth() +
      "px;}";
    quickshop_css +=
      "body.opening-quickshop .is-sticky .header-sticky{width: calc(100% - " +
      $.position.scrollbarWidth() +
      "px);}";
    quickshop_css += "</style>";
  ...
  $(document).on(
    ...
    function (e, variation) {
      ...
      if (
        typeof variation.discount_percent != "undefined" &&
        variation.discount_percent
      ) {
        wrapper.find(".ts-discount-percent").removeClass("hidden");
        wrapper
          .find(".ts-discount-percent span")
          .html(variation.discount_percent);
      } else {
  ...
  $(document).on(
    "change",
    ".product form.variations_form .variations select",
    function () {
      var data = $(this).val();
      var dataAttr = data.length
        ? $(this)
  ...
  if (
    typeof ecomall_params != "undefined" &&
    ecomall_params.ajax_add_to_cart == 1 &&
      ...
      function (e) {
        ...
        $.post(product_url, data, function (result) {
          ...
          if (result.find(".woocommerce-notices-wrapper .is-error").length) {
            error = result
              .find(
                ".woocommerce-notices-wrapper .wc-block-components-notice-banner__content:first"
              )
              .html();
          }
          form
  ...
  $(".ts-sidebar .widget-title-wrapper .block-control").on(
    "click",
    function (e) {
      e.preventDefault();
      $(this).toggleClass("active");
  ...
  $(window).on("resize ts_set_top_shop_elements", function () {
    var top = 0;
    top += $(".is-sticky .header-sticky").length
      ? $(".is-sticky .header-sticky").height()
      : 0;
  ...
  function lazyload_slider_middle_navigation_position(img) {
    if (
      img.parents(".swiper").length &&
      !img.parents(".swiper.lazy-recalc-nav-pos").length &&
      img.parents(".swiper-slide-active").length
    ) {
      img.parents(".swiper").addClass("lazy-recalc-nav-pos");
      $(window).trigger("ts_slider_middle_navigation_position", [
        img.parents(".swiper"),
      ]);
  ...
  if (!$("body").hasClass("woocommerce-cart")) {
    $(document.body).on("adding_to_cart", function (e, $button, data) {
      if (wc_add_to_cart_params.cart_redirect_after_add == "no") {
        if (
          typeof ecomall_params != "undefined" &&
          ecomall_params.add_to_cart_effect == "show_popup" &&
          ...
          $.ajax({
            ...
            url: ecomall_params.ajax_url,
            data: {
              action: "ecomall_load_product_added_to_cart",
              product_id: product_id,
              security: ecomall_params.addtocart_nonce,
            },
            success: function (response) {
    ...
    $(document.body).on(
      ...
      function (e, fragments, cart_hash, $button) {
        if (
          typeof ecomall_params != "undefined" &&
          ecomall_params.show_cart_after_adding == 1
        ) {
            ...
            if (cart.length == 1) {
              ...
              if (product_img.length == 1) {
                ...
                var imgclone_small_height = product_img.width()
                  ? (60 * product_img.height()) / product_img.width()
                  : 60;
                ...
                if (!cart_in_sticky) {
                  $("body,html").animate({ scrollTop: "0px" }, effect_time);
                }
  ...
  $(document).on("change", ".ts-tiny-cart-wrapper .qty", function () {
    ...
    $.ajax({
      ...
      url: ecomall_params.ajax_url,
      data: {
        action: "ecomall_update_cart_quantity",
        qty: qty,
        cart_item_key: cart_item_key,
        security: ecomall_params.update_cart_nonce,
      },
      success: function (response) {
        if (!response) {
          return;
  ...
  function update_filter_area_main_content_height() {
    if (
      !$("#ts-filter-widget-area").length ||
      $("#main-content").hasClass("style-top")
    ) {
      return;
    }
    if ($("#ts-filter-widget-area").hasClass("active")) {
      filter_sidebar_interval = setInterval(function () {
        $("#main-content").css(
          "min-height",
          $("#ts-filter-widget-area .filter-widget-area").height() +
            ($(".before-loop-wrapper").length
              ? $(".before-loop-wrapper").outerHeight(true)
              : 0) +
            20
        );
      }, 1000);
  ...
  update_filter_area_main_content_height();
  $(".filter-widget-area-button a, .filter-widget-area-button + .overlay").on(
    "click",
    function () {
...
function ts_sticky_menu() {
  ...
  if (
    jQuery("body").hasClass("display-vertical-menu") &&
    jQuery(".ts-header nav.vertical-menu").length
  ) {
...
function ts_update_tini_wishlist() {
  ...
  jQuery.ajax({
    ...
    url: ecomall_params.ajax_url,
    data: {
      action: "ecomall_update_tini_wishlist",
      security: ecomall_params.wishlist_nonce,
    },
    success: function (response) {
  ...
  jQuery(
    ...
    if (search_cache_data[search_string]) {
      if (!is_sidebar) {
        ...
        search_result_container_sidebar.html(
          search_cache_data[search_string][0]
        );
    ...
    search_timeout = setTimeout(function () {
      ...
      jQuery.ajax({
        ...
        url: ecomall_params.ajax_url,
        data: {
          action: "ecomall_ajax_search",
          search_string: search_string,
          category: category,
          security: ecomall_params.search_nonce,
        },
        ...
        success: function (response) {
          if (response != "") {
            ...
            if (response.search_string == search_string) {
              search_cache_data[search_string] = [
                response.html,
                response.count,
              ];
              if (!is_sidebar) {
...
(function ($) {
  var defaults = {
    ...
    resizer = function () {
      windowHeight = $window.height();
      for (var i = 0; i < sticked.length; i++) {
        sticked[i].stickyWrapper.css(
          "height",
          sticked[i].stickyElement.outerHeight()
        );
        sticked[i].topSpacing = $("#wpadminbar").length
          ? $("#wpadminbar").height()
          : 0;
      }
    },
...
})(jQuery); /*! elementor - v3.20.0 - 13-03-2024 */
(() => {
  "use strict";
  var e,
    r,
